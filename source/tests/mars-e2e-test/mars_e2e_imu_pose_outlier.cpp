...
///
class mars_e2e_imu_pose_outlier : public testing::Test
{
public:
  ...
  void LoadInstancesAndParams()
  {
    ...
    pose_sensor_sptr->const_ref_to_nav_ = true;
  ...
  std::vector<mars::BufferEntryType> LoadData()
  {
    ...
    for (auto it = measurement_data.begin(); it != measurement_data.end(); it++)
    {
      if (it->sensor_handle_ == pose_sensor_sptr && it->timestamp_ > mars::Time(5 * 60) &&
          (it - measurement_data.begin()) % 2)
      {
        mars::PoseMeasurementType* meas = static_cast<mars::PoseMeasurementType*>(it->data_.measurement_.get());
        meas->position_ = Eigen::Vector3d(1e5, 1e5, 1e5);

        it->data_.set_measurement(std::make_shared<mars::PoseMeasurementType>(*meas));
      }
    }
...
TEST_F(mars_e2e_imu_pose_outlier, END_2_END_IMU_POSE_UPDATE_OUTLIER)
{
  ...
  // core_logic.buffer_.PrintBufferEntries();
}

TEST_F(mars_e2e_imu_pose_outlier, END_2_END_IMU_POSE_UPDATE_OUTLIER_TAGGING)
{
  mars::BufferEntryType latest_result;
  core_logic.buffer_.get_entry_at_idx(295, &latest_result);
  EXPECT_TRUE(latest_result.metadata_ == mars::BufferMetadataType::invalid);
  core_logic.buffer_.get_entry_at_idx(284, &latest_result);
  EXPECT_TRUE(latest_result.metadata_ == mars::BufferMetadataType::invalid);
  core_logic.buffer_.get_entry_at_idx(273, &latest_result);
  EXPECT_TRUE(latest_result.metadata_ == mars::BufferMetadataType::invalid);
}

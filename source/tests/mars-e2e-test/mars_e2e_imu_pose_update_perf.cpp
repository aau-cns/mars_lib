...
TEST_F(mars_e2e_imu_pose_update, END_2_END_IMU_POSE_UPDATE_PERF)
{
  ...
  {
    Reset();
    double t_start = mars::Time::get_time_now().get_seconds();
    RunFilter();
    double t_end = mars::Time::get_time_now().get_seconds();

    double t_duration = t_end - t_start;
    std::cout << "Single itteration " << k << " time: " << t_duration << std::endl;

    time_accum += t_duration;
  }

  std::cout << "Average processing time over " << num_iteration << " iterations:" << time_accum / num_iteration
            << std::endl;

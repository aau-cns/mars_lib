...
class mars_vision_sensor_test : public testing::Test
{
public:
};

TEST_F(mars_vision_sensor_test, CTOR_POSE_SENSOR)
{
  mars::CoreState core_states;
  std::shared_ptr<mars::CoreState> core_states_sptr = std::make_shared<mars::CoreState>(core_states);
  mars::VisionSensorClass vision_sensor("Vision", core_states_sptr);
}

TEST_F(mars_vision_sensor_test, VISION_SENSOR_MEASUREMENT)
{
  ...
  mars::VisionMeasurementType measurement(position, orientation);
...
TEST_F(mars_vision_sensor_test, VISION_SENSOR_INIT)
{
  Eigen::Vector3d position;               // Position [x y z]
  Eigen::Quaternion<double> orientation;  // Quaternion [W X Y Z]
  ...
  EXPECT_DEATH(vision_sensor.Initialize(1, std::make_shared<mars::VisionMeasurementType>(measurement),
                                        std::make_shared<mars::CoreType>()),
               "");
}

TEST_F(mars_vision_sensor_test, VISION_UPDATE)
{
  Eigen::Vector3d position;               // Position [x y z]
  Eigen::Quaternion<double> orientation;  // Quaternion [W X Y Z]
  ...
  Eigen::Matrix<double, prior_core_state.size_error_ + 15, prior_core_state.size_error_ + 15> prior_cov;
  prior_cov.setIdentity();

  // TODO no update without init
  //  int timestamp = 1;
  //  mars::BufferDataType test =
  //      vision_sensor.CalcUpdate(timestamp, std::make_shared<mars::VisionMeasurementType>(measurement),
  //      prior_core_state,
